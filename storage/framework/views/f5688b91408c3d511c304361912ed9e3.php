<script src="./libs/apexcharts/dist/apexcharts.min.js?1744816593" defer></script>
    <script src="./libs/jsvectormap/dist/jsvectormap.min.js?1744816593" defer></script>
    <script src="./libs/jsvectormap/dist/maps/world.js?1744816593" defer></script>
    <script src="./libs/jsvectormap/dist/maps/world-merc.js?1744816593" defer></script>
    <!-- END PAGE LIBRARIES -->
    <!-- BEGIN GLOBAL MANDATORY SCRIPTS -->
    <script src="<?php echo e(asset('vendors/tabler/js/tabler.min.js')); ?>" defer></script>
    <!-- END GLOBAL MANDATORY SCRIPTS -->
    <!-- BEGIN DEMO SCRIPTS -->
    <script src="<?php echo e(asset('vendors/tabler/js/demo.min.js')); ?>" defer></script>
    <!-- END DEMO SCRIPTS -->
    <!-- BEGIN PAGE SCRIPTS --><?php /**PATH /Users/administrator/Herd/damis_back_office/resources/views/layouts/tabler/js_files_links.blade.php ENDPATH**/ ?>