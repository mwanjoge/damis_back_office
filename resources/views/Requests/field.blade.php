...
<div class="mb-3">
    ...
    <select name="embassy_id" id="embassy_id" class="form-control" required>
        @foreach($embassies as $embassy)
            <option value="{{ $embassy->id }}" {{ (old('embassy_id', $request->embassy_id ?? '') == $embassy->id) ? 'selected' : '' }}>
...
<div class="mb-3">
    ...
    <select name="service_provider_id" id="service_provider_id" class="form-control" required>
        @foreach($serviceProviders as $provider)
            <option value="{{ $provider->id }}" {{ (old('service_provider_id', $request->service_provider_id ?? '') == $provider->id) ? 'selected' : '' }}>
...
<div class="mb-3">
    ...
    <select name="type" id="type" class="form-control" required>
        <option value="Diaspora" {{ old('type', $request->type ?? '') == 'Diaspora' ? 'selected' : '' }}>Diaspora</option>
        <option value="Domestic" {{ old('type', $request->type ?? '') == 'Domestic' ? 'selected' : '' }}>Domestic</option>
    </select>
</div>
<div class="mb-3">
...
<div class="mb-3">
    ...
    <input type="text" name="tracking_number" id="tracking_number" class="form-control" value="{{ old('tracking_number', $request->tracking_number ?? '') }}" required>
</div>
<div class="mb-3">
    <label for="is_approved" class="form-label">Is Approved</label>
    <input type="checkbox" name="is_approved" id="is_approved" value="1" {{ old('is_approved', $request->is_approved ?? false) ? 'checked' : '' }}>
</div>
<div class="mb-3">
    <label for="is_paid" class="form-label">Is Paid</label>
    <input type="checkbox" name="is_paid" id="is_paid" value="1" {{ old('is_paid', $request->is_paid ?? false) ? 'checked' : '' }}>
</div>
<div class="mb-3">
    <label for="total_cost" class="form-label">Total Cost</label>
    <input type="number" step="0.01" name="total_cost" id="total_cost" class="form-control" value="{{ old('total_cost', $request->total_cost ?? 0) }}" required>
</div>
<div class="mb-3">
    ...
    <select name="sent_status" id="sent_status" class="form-control" required>
        <option value="sent" {{ old('sent_status', $request->sent_status ?? '') == 'sent' ? 'selected' : '' }}>Sent</option>
        <option value="failed" {{ old('sent_status', $request->sent_status ?? '') == 'failed' ? 'selected' : '' }}>Failed</option>
    </select>
</div>